...
data = json.load(open(f'{ROOT}/data/words-hy.json', encoding='utf-8'))
words = data['words']
if len(words) < 4000:
...
alphabet = json.load(open(f'{ROOT}/data/alphabet-hy.json', encoding='utf-8'))
if len(alphabet) != 40:
    errors.append(f'в алфавите {len(alphabet)} букв вместо 40')

json.load(open(f'{ROOT}/data/mnemonics-hy.json', encoding='utf-8'))
...
EXPLICIT_RU = re.compile(r'женская грудь|\bпенис|половой член|вагин|ягодиц|задниц|порногр|проститу|бордел|презерватив|мастурб|оргазм|эрекц|\bсперм|\bсекс\b|(оральн|анальн|вагинальн|групп\w*)\w*\s+секс', re.I)
for _w in words:
